...
{ LOG(("%s(%lx)\n", __func__, c));
  ...
  fb_1bpp_fill( fb_plot_ctx.x0, fb_plot_ctx.y0
...
static bool fb_1bpp_text( int x, int y
...
static bool fb_1bpp_bitmap_tile( int x, int y
                               , int width, int height
...
{ unsigned long xf,yf,wf,hf;
  ...
  if (!(repeat_x || repeat_y))  /* Not repeating at all, so just pass it on */
  ...
  { for(yf = 0;yf < height; yf += bitmap->height)
    { if(width > xf+bitmap->width) { wf = width-(xf+bitmap->width); }
                              else { wf=bitmap->width;  }

      if(height > yf+bitmap->height) { hf = height-(yf+bitmap->height); }
                                else { hf=bitmap->height;  }
...
static bool fb_1bpp_path( const float *p, unsigned int n
                        , colour fill, float width
