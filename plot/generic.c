 ...
 */
static bool establish_crossing_value( int x, int y
                                    , int x0, int y0
                                    , int x1, int y1 )
{ bool v1 = (x == x0 && y == y0); /* whether we're crossing 1st vertex */
  bool v2 = (x == x1 && y == y1); /* whether we're crossing 2nd vertex */

  if ((v1 && (y0 < y1)) || (v2 && (y1 < y0)))  /* crossing top vertex */
  { return true;
  }
  else if (!v1 && !v2)  /* Intersection with current y level is not at a vertex. * Normal crossing. */
  { return true;
  }
  return false;
}
 ...
 */
static bool find_span( const int *p, int n
                     , int x, int y
                     , int *x0, int *x1 )
{ int i;
  int p_x0, p_y0;
  ...
  *x0 = *x1 = INT_MAX;
  ...
  do /* search row for next span, returning it if one exists */
  { if (!found_span_start) /* reset endpoint info, if valid span endpoints not found */
	  *x0 = INT_MAX;
	  *x1 = INT_MAX;
...
{ int srcx  = srcbox->x0;
  ...
  { if (srcy > dsty)
    { for (hloop = height; hloop > 0; hloop--)
     ...
     { srcptr += height * nsfb->panlen;
       dstptr += height * nsfb->panlen;
...
{ unsigned int seg_loop;
  ...
  { t = (double)seg_loop / (double)pointc;
    ...
    if ((point[cur_point].x != point[cur_point - 1].x)
     || (point[cur_point].y != point[cur_point - 1].y)) cur_point++;
  }
...
{ const nsfbPlotterFns * table= NULL;
  ...
  { case NSFB_FMT_XBGR8888: /* 32bpp Unused Blue Green Red */
   ...
   break;
    ...
    case NSFB_FMT_RGB888 + 1234 : /* 24 bpp Alpha Red Green Blue */
#ifdef ENABLE_24_BPP
