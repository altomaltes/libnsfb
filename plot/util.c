 ...
 */
  ...
  if (rect->x0 > clip->x1) rect->x0 = clip->x1;

  if (rect->x1 < clip->x0) rect->x1 = clip->x0;
  if (rect->x1 > clip->x1) rect->x1 = clip->x1;

  if (rect->y0 < clip->y0) rect->y0 = clip->y0;
  if (rect->y0 > clip->y1) rect->y0 = clip->y1;

  if (rect->y1 < clip->y0) rect->y1 = clip->y0;
...
{ char region1;
  ...
  { if (region1 & region2)               /* line lies entirely outside the clipping rectangle */
    { return false;
    }

    if (region1)                          /* first point */
    { if (region1 & POINT_BELOW_REGION)   /* divide line at bottom */
      { line->x0 = (line->x0 + (line->x1 - line->x0)
                 * (clip->y1 - 1 - line->y0) / (line->y1 - line->y0));
        line->y0 = clip->y1 - 1;
      }
      else if (region1 & POINT_ABOVE_REGION)                            /* divide line at top */
      { line->x0 = (line->x0 + (line->x1 - line->x0)
                 * (clip->y0 - line->y0) / (line->y1 - line->y0));
        line->y0 = clip->y0;
      }
      else if (region1 & POINT_RIGHTOF_REGION)                            /* divide line at right */
      { line->y0 = (line->y0 + (line->y1 - line->y0)
                 * (clip->x1  - 1 - line->x0) / (line->x1 - line->x0));
        line->x0 = clip->x1 - 1;
      }
      else if (region1 & POINT_LEFTOF_REGION)                            /* divide line at left */
      { line->y0 = (line->y0 + (line->y1 - line->y0)
    ...
    { if (region2 & POINT_BELOW_REGION)                 /* divide line at bottom*/
      { line->x1 = (line->x0 + (line->x1 - line->x0) *
                   (clip->y1  - 1 - line->y0) / (line->y1 - line->y0));
                     line->y1 = clip->y1 - 1;
      }
      else if (region2 & POINT_ABOVE_REGION)            /* divide line at top*/
      { line->x1 = (line->x0 + (line->x1 - line->x0) *
                 (clip->y0 - line->y0) / (line->y1 - line->y0));
        line->y1 = clip->y0;
      }
      else if (region2 & POINT_RIGHTOF_REGION)          /* divide line at right*/
      { line->y1 = (line->y0 + (line->y1 - line->y0) *
                     (clip->x1  - 1 - line->x0) / (line->x1 - line->x0));
                                line->x1 = clip->x1 - 1;
      }
      else if (region2 & POINT_LEFTOF_REGION)                          /* divide line at right*/
      { line->y1 = (line->y0 + (line->y1 - line->y0) *
                           (clip->x0 - line->x0) / (line->x1 - line->x0));
        line->x1 = clip->x0;
      }
