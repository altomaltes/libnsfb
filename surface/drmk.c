...
{  void * buf1; /**< First display buffer. */
   ...
   /* 1: It was the last chunk to flush. (It can't be a bi tfield because when it's cleared from IRQ Read-Modify-Write issue might occ
      ur)*/
...
{  int fd;
   uint32_t conn_id
          , enc_id, crtc_id, plane_id, crtc_idx;
   uint32_t width
          , height;
 ...
 *
 */
static uint32_t getPlanePropertyId(const char *name)
{  uint32_t i;

   dbg("Find plane property: %s", name);

   for( i = 0
      ; i < drm_dev.count_plane_props
      ; ++i)
 ...
 *
 */
static uint32_t get_crtcPropertyId(const char *name)
{  uint32_t i;
 ...
 *
 */
static uint32_t getConnPropertyId(const char *name)
{  uint32_t i;
 ...
 *
 */
static int drmGetPlaneProps(void)
{  uint32_t i;

   drmModeObjectPropertiesPtr props=
      drmModeObjectGetProperties( drm_dev.fd
                                , drm_dev.plane_id
 ...
 *
 */
static int drmGetCrtcProps( void )
{  uint32_t i;
...
*/
static int drmGetConnProps(void)
{  uint32_t i;
...
*/
static int drmAdd_planeProperty(const char *name, uint64_t value)
{  int ret;
   uint32_t prop_id = getPlanePropertyId(name);
...
*/
static int drmAdd_crtcProperty( const char *name
                                 , uint64_t value )
{  if (!get_crtcPropertyId(name))
   {  err("Couldn't find crtc prop %s", name);
      return( -1 );
   }

   if (drmModeAtomicAddProperty(drm_dev.req, drm_dev.crtc_id, get_crtcPropertyId(name), value) < 0)
...
*/
static int drmAdd_connProperty(const char *name, uint64_t value)
{  int ret;
   uint32_t prop_id = getConnPropertyId(name);
   ...
   ret = drmModeAtomicAddProperty(drm_dev.req, drm_dev.conn_id, getConnPropertyId(name), value);
   if (ret < 0)
   {  err("drmModeAtomicAddProperty (%s:%" PRIu64 ") failed: %d", name, value, ret);
      return ret;
   }
...
*/
static int drmDmabufSetPlane( struct drm_buffer *buf )
{  int ret;
   static int first = 1;
   ...
   if (first)
   {  drmAdd_connProperty("CRTC_ID", drm_dev.crtc_id);

      drmAdd_crtcProperty("MODE_ID", drm_dev.blob_id);
      drmAdd_crtcProperty("ACTIVE", 1);
   ...
   drmAdd_planeProperty( "FB_ID", buf->fb_handle);
   drmAdd_planeProperty( "CRTC_ID", drm_dev.crtc_id);
   drmAdd_planeProperty( "SRC_X", 0);
   drmAdd_planeProperty( "SRC_Y", 0);
   drmAdd_planeProperty( "SRC_W", drm_dev.width << 16);
   drmAdd_planeProperty( "SRC_H", drm_dev.height << 16);
   drmAdd_planeProperty( "CRTC_X", 0);
   drmAdd_planeProperty( "CRTC_Y", 0);
   drmAdd_planeProperty( "CRTC_W", drm_dev.width);
   drmAdd_planeProperty( "CRTC_H", drm_dev.height);
...
*/
static int findPlane( unsigned int fourcc
                      , uint32_t * plane_id
                      , uint32_t crtc_id
                      , uint32_t crtc_idx )
...
*/
static int drmFindConnector(void)
{  drmModeConnector *conn = NULL;
   ...
   for( i = 0 /** find all available connectors */
   ...
   for( i= 0
      ...
      ; i++ )
   {  if (!(enc = drmModeGetEncoder(drm_dev.fd, res->encoders[i])))
      {  continue;
      }
   ...
   else   /* Encoder hasn't been associated yet, look it up */
   {  for( i= 0
...
*/
static int drmOpenPriv(const char *path)
{  int fd, flags;
   uint64_t has_dumb;
   ...
   if (( flags = fcntl( fd, F_GETFD)) < 0   /** set FD_CLOEXEC flag */
         || fcntl( fd, F_SETFD
                   , flags | FD_CLOEXEC) < 0)
...
*/
static int drmSetup( unsigned int fourcc )
{  drm_dev.fd = drmOpenPriv( DRM_CARD );

   if ( drm_dev.fd < 0)
   {  return( drm_dev.fd );
   }

   /* if ( drmSetClientCap( drm_dev.fd
   ...
   */
   ...
   if (!(drm_dev.plane = drmModeGetPlane(drm_dev.fd, drm_dev.plane_id)))
   {  err("Cannot get plane");
      goto err;
   }

   if (!(drm_dev.crtc = drmModeGetCrtc(drm_dev.fd, drm_dev.crtc_id)))
   {  err("Cannot get crtc");
      goto err;
   }

   if (!(drm_dev.conn = drmModeGetConnector(drm_dev.fd, drm_dev.conn_id)))
   ...
   drm_dev.drm_event_ctx.page_flip_handler = pageFlipHandler;
 ...
 *
 */
static int drmAllocateDumb( struct drm_buffer *buf )
 ...
 *
 */
static int drmSetupBuffers(void)
{  int ret;

   if(( ret= drmAllocateDumb(&drm_dev.drm_bufs[0])))   /* Allocate DUMB buffers */
   { return( ret );
   }

   if(( ret= drmAllocateDumb(&drm_dev.drm_bufs[1])))
   { return( ret );
   }
 ...
 *
 */
void drmWaitVsync( lv_disp_drv_t * disp_drv )
{  int ret;
   fd_set fds;
   FD_ZERO(&fds);
 ...
 *
 */
void drmFlush(       lv_disp_drv_t * disp_drv
...
{  struct drm_buffer *fbuf = drm_dev.cur_bufs[1];
   ...
   */
   ...
   if (drm_dev.req)
   {  drmWaitVsync(disp_drv);
   }

   /* show fbuf plane */
   if ( drmDmabufSetPlane(fbuf))
   {  err("Flush fail");
      return;
   }
   dbg("Flush done");
...
// lv_disp_flush_ready(disp_drv); FORDWARD
}
...
void drmGetSizes( lv_coord_t * width
                , lv_coord_t * height
                , uint32_t   * dpi )
