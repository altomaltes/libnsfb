...
{ struct cjpeg_source_struct pub;  /* public fields */
  ...
  JMETHOD( void
         , read_pixel
         , (tga_source_ptr sinfo)); /* Pointer to routine to extract next Targa pixel from input file */
  U_CHAR tga_pixel[4];              /* Result of read_pixel is delivered here: */
...
METHODDEF(JDIMENSION) get_8bit_gray_row ( j_compress_ptr cinfo
                                        , cjpeg_source_ptr sinfo )
...
METHODDEF(JDIMENSION) get_16bit_row (j_compress_ptr cinfo, cjpeg_source_ptr sinfo)
{
  ...
  for (col = cinfo->image_width; col > 0; col--) {
     ...
     *     xRRRRRGGGGGBBBBB
     */
    ptr[2] = (JSAMPLE) c5to8bits[t & 0x1F]; t >>= 5;
    ptr[1] = (JSAMPLE) c5to8bits[t & 0x1F]; t >>= 5;
    ptr[0] = (JSAMPLE) c5to8bits[t & 0x1F]; ptr += 3;
  }
  return 1;
}
