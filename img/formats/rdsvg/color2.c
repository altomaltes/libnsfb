...
const struct svgtiny_named_color *svgtiny_color_lookup(const char *s, unsigned int len) {
  ...
  if (!cmp || !strcmp(lowerStr, sSortedColors[mid].name)) 
...
test() {
  ...
  const struct svgtiny_named_color *aliceblueS = svgtiny_color_lookup(aliceblue, (unsigned int)strlen(aliceblue));
  assert(aliceblueS); // first

  const char *yellowgreen = "yellowgreen";
  const struct svgtiny_named_color *yellowgreenS = svgtiny_color_lookup(yellowgreen, (unsigned int)strlen(yellowgreen));
  assert(yellowgreenS); // last

  const char *a000 = "a000";
  const struct svgtiny_named_color *a000S = svgtiny_color_lookup(a000, (unsigned int)strlen(a000));
  assert( ! a000S); // before first

  const char *zztop = "zztop";
  const struct svgtiny_named_color *zztopS = svgtiny_color_lookup(zztop, (unsigned int)strlen(zztop));
