...
static void svgtiny_parse_font_attributes(dom_element *node,
		struct svgtiny_parse_state *state);
static void svgtinyParseTransform_attributes(dom_element *node,
		struct svgtiny_parse_state *state);
static svgtiny_code svgtiny_add_path(float *p, unsigned int n,
		struct svgtiny_parse_state *state);
static void _svgtinyParseColor(const char *s, svgtiny_colour *c,
		struct svgtiny_parse_state *state);
...
/**
 * Create a new SvgtinyDiagram structure.
 */
...
/**
 * Parse a block of memory into a SvgtinyDiagram.
 */

svgtiny_code svgtiny_parse_inner(struct SvgtinyDiagram *diagram,
		...
		struct svgtiny_parse_state *state)
{
	...
	if (exc == DOM_NO_ERR && attr != NULL) {
		transform = strndup(dom_string_data(attr),
				    dom_string_byte_length(attr));
		svgtinyParseTransform(transform, &state->ctm.a, &state->ctm.b,
...
/**
 * Add a path to the SvgtinyDiagram.
 */
...
/**
 * Add a svgtiny_shape to the SvgtinyDiagram.
 */
 ...
 *                                                                            *
\* ========================================================================= **/
ANSIC DeviceImageRec * loadImgFile( const char * fname  )
{
}
