		...
		struct svgtiny_parse_state *state)
{
	...
	if (exc == DOM_NO_ERR && attr != NULL) {
		...
		svgtinyParseTransform(s, &a, &b, &c, &d, &e, &f);
		free(s);
		#ifdef GRADIENT_DEBUG
	...
	if (exc == DOM_NO_ERR && stops != NULL) {
		...
		for (stopnr = 0; stopnr < listlen; ++stopnr) {
			...
			exc = dom_element_get_attribute(stop,
							...
							&attr);
			if (exc == DOM_NO_ERR && attr != NULL) {
				svgtinyParseColor(attr, &color, state);
				dom_string_unref(attr);
			}
			exc = dom_element_get_attribute(stop,
  ...
  { float x, y, r;
	...
	fprintf(stderr, "object bbox: (%g %g) (%g %g)\n",
	 ...
	 { gradient_x0= object_x0
	              +	svgtiny_parse_length( state->gradient_x1,object_x1 - object_x0, *state);

	  	gradient_y0= object_y0 +	svgtiny_parse_length( state->gradient_y1, object_y1 - object_y0, *state );
	  	gradient_x1= object_x0 +	svgtiny_parse_length( state->gradient_x2,	object_x1 - object_x0, *state );
		  gradient_y1= object_y0 +	svgtiny_parse_length( state->gradient_y2,	object_y1 - object_y0, *state );
	 }
 	else
 	{ gradient_x0= svgtiny_parse_length(state->gradient_x1,	state->viewport_width , *state );
 	 	gradient_y0= svgtiny_parse_length(state->gradient_y1,	state->viewport_height, *state );
 	 	gradient_x1= svgtiny_parse_length(state->gradient_x2,	state->viewport_width , *state );
 	 	gradient_y1= svgtiny_parse_length(state->gradient_y2, state->viewport_height, *state );
 	}
