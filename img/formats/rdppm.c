...
{ struct cjpeg_source_struct pub; /* public fields */
  ...
  int buffer_width;          /* width of I/O buffer */
...
METHODDEF(JDIMENSION) get_scaled_gray_row( j_compress_ptr cinfo
...
METHODDEF(JDIMENSION) get_scaled_rgb_row( j_compress_ptr cinfo
                                        , cjpeg_source_ptr sinfo )
{ ppm_source_ptr source = (ppm_source_ptr) sinfo;
  register JSAMPROW ptr;
  register U_CHAR * bufferptr;
...
METHODDEF(JDIMENSION) get_word_rgb_row (j_compress_ptr cinfo, cjpeg_source_ptr sinfo)
{
  ...
  { register int temp;
    ...
    temp= UCH(*bufferptr++); temp |= UCH(*bufferptr++) << 8; *ptr++ = rescale[temp];       
    temp= UCH(*bufferptr++); temp |= UCH(*bufferptr++) << 8; *ptr++ = rescale[temp];
    temp= UCH(*bufferptr++); temp |= UCH(*bufferptr++) << 8; *ptr++ = rescale[temp];
  }
  return 1;
...
METHODDEF(void) start_input_ppm( j_compress_ptr cinfo
                               , cjpeg_source_ptr sinfo )
{ ppm_source_ptr source = (ppm_source_ptr) sinfo;
  int c;
  ...
  if ( need_iobuffer ) 
  { source->buffer_width = (int) w * cinfo->input_components *
  ...
  else    /* Need to translate anyway, so make a separate sample buffer. */
  ...
  { INT32 val, half_maxval;
    ...
    source->rescale = (JSAMPLE *)
      (*cinfo->mem->alloc_small) ((j_common_ptr) cinfo, JPOOL_IMAGE,
                                  (int) (((long) maxval + 1L) * sizeof (JSAMPLE)));
