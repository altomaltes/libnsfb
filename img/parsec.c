 ...
 *     DATE: jul 2003
 *
 *  DESCRIPCION: Human readable colors
 *
 */
...
static colorNameRec colorNames[]=
...
{ colorNameRec * ptr;
 // char tmp[32];
  ...
  for( ptr= colorNames
     ; ptr->colorName
     ; ptr ++ )
  { if ( !strcmp( name, ptr->colorName ))
    {
      *r= ptr->r;
      *g= ptr->g;
      *b= ptr->b;
      return( 127 ); /* Mask purposes */
  } }

  *r= *g= *b= 128; /* Return an "always good" color */
  return( 0 );     /* Void mask */

}

unsigned long parsedColor( const char * name )
{ unsigned char r, g, b;
  unsigned long index;

  parseColorName( name, &r, &g, &b );
